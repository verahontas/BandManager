...
namespace EasyRehearsalManager.Web.Models
{
    public class ReservationService : IReservationService
    {
        ...
        /// <summary>
        /// This constructor is only required for Mock testing.
        /// </summary>
        ...
        public bool UpdateReservationEquipmentTable(int reservationId, int studioId, Dictionary<string, bool> equipments)
        {
            ...
            {
                if (eq.Value == true) //if we booked this equipment, but it wasn't booked before
                {
                    //if it is already in the database, we don't have to do anything
                    //if (ReservationEquipmentPairs.FirstOrDefault(l => l.ReservationId == reservationId && l.EquipmentName == eq.Key) != null) { }
                    //if it isn't in the database yet, then we have to add to it
                ...
                else //if the equipment was already booked and now we want to give up OR equipment wasn't booked at all
                {
                    //if equipment wasn't booked at all, then we don't have to do anything
                    //if (ReservationEquipmentPairs.FirstOrDefault(l => l.ReservationId == reservationId && l.EquipmentName == eq.Key && l.StudioId == studioId) == null) { }
                    //if it was booked and now we want to give it up
                    if (ReservationEquipmentPairs.FirstOrDefault(l => l.ReservationId == reservationId && l.EquipmentName == eq.Key && l.StudioId == studioId) != null)
                    {
